...
class Case1PromptSelector:
    ...
    def visualize_selection(self, candidate_positions, selected_positions, cluster_info):
        ...
        plt.title('Tx Position Selection')
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.show()

if __name__ == "__main__":
    selector = Case1PromptSelector(layout_shape=(H, W))
    print("Generating candidate Tx positions...")
    candidate_positions = selector.generate_feasible_tx_positions(n_positions=N)
    print(f"Generated {len(candidate_positions)} feasible positions")
    k = 3
    print(f"\nSelecting {k} representative positions...")
